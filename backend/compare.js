...
function compareDirs(dir1, dir2) {
  ...
  allFiles.forEach(file => {
    ...
    if (exists1 && !exists2) {
      ...
      const content1 = fs.readFileSync(filePath1, 'utf-8').split('\n');
      const content2 = fs.readFileSync(filePath2, 'utf-8').split('\n');
      const diffs = [];

      const max = Math.max(content1.length, content2.length);
      for (let i = 0; i < max; i++) {
        if (content1[i] !== content2[i]) {
          diffs.push({ line: i + 1, old: content1[i] || '', new: content2[i] || '' });
        }
      }
